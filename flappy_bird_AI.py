...
def main():
    ...
    player_pos = [100, 350]
    gravity = 1.5
    speed = 0 #Resistencia al salto cuanto mayor es menor es el intervalo de salto
    jump = -10 #Mientras menor sea el jump mayor es el arco de salto 
    #speed_increase = 1 #A mayor s.increase mayor es speed y baja más 
    is_rising = False
    ...
    while run:
        ...
        if (pipe_pos == 100) and (score % 10 == 0):
            phantom_pos = [random.randint(250, 450), random.randint(200, 400)]
            phantom_count = 0
        ...
        if 'phantom_pos' in locals():
            if phantom_pos[0] < 500:
                ...
                phantom_pos[0] -= 10
                phantom_pos[1] += 10 * math.sin(phantom_pos[0] / 100)  # movimiento sinusoidal en Y
                new_y_pos = phantom_pos[1] + 10 * math.sin(phantom_pos[0] / 100)
                if new_y_pos < phantom_pos[1]:
                    is_rising = True
                else:
                    is_rising = False
        ...
        # # INTENTO DE JUGADOR AUTOMÁTICO

        # Ideas de la IA:
        #     - Hacer que cuando el hueco entre los tubos sea pequeño la velocidad se maximice a 10/15 para que no choque
        #     con el pipe de arriba (Hecho)
        #     - Ajustar los parametros para que pueda coger mas cerezas 
        #     - Modificar el codigo para que tambien esquive al fantasma cuando este arriba del personaje

        #print(pipe_height[0], player_pos[1])
        # if (player_pos[1] > pipe_height[1] -30):
        ...
        #Para la detección de los tubos: 
        if (player_pos[1] > 600):
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))

        if (pipe_height[1] - pipe_height[0] > 150):
            if ((player_pos[1] > pipe_height[1] -30) and (pipe_pos - player_pos[0] < 220) and speed > -15):
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))
        else:
            if ((player_pos[1] > pipe_height[1] -30) and (pipe_pos - player_pos[0] < 220) and speed > -12): 
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))

        # Para que cuando el pajaro aquiera una gran velocidad al caer se pulse jump y suavice el descenso 
        # que es algo que un jugador normal haría
        if (speed > 13):
            pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))
       
        #Para que el pajaro salte dependiendo de la posicion del fantasma lo ponemos en rango positivo para que una vez superado el objeto no repita la accion
        # if 'phantom_pos' in locals():
        #     if ((0 < phantom_pos[1] - player_pos[1] < 150) and (0 < phantom_pos[0] - player_pos[0] < 70) and (phantom_count < 5)):
        #         pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))
        #         phantom_count += 1 
        #     if ((0 > phantom_pos[1] - player_pos[1] > -100) and (0 < phantom_pos[0] - player_pos[0] < 70) and (speed > -20)):
        #         pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))

        #UPDATE
        if 'phantom_pos' in locals():
            if ((0 < phantom_pos[1] - player_pos[1] < 150) and (0 < phantom_pos[0] - player_pos[0] < 120) and (is_rising == True)):
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))
            if ((0 > phantom_pos[1] - player_pos[1] > -100) and (0 < phantom_pos[0] - player_pos[0] < 120) and (is_rising == False)):
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))


        # Para la detección y toma de la cherry
        if 'cherry_pos' in locals():
            if ((70 < cherry_pos[1]) and (player_pos[1] - cherry_pos[1] > 0) and (cherry_pos[0] > player_pos[0]) and (player_pos[0] - pipe_pos > 10) and (speed > -20)):
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))
            elif ((100 < cherry_pos[1]) and (player_pos[1] - cherry_pos[1] > 0) and (cherry_pos[0] > player_pos[0]) and (player_pos[0] - pipe_pos < -10) and (speed > -20)):
                pygame.event.post(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_SPACE}))
